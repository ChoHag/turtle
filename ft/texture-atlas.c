...
{
    ivec4_t region = texture_atlas_get_region( self, 5, 5 );
                   ...
                   const size_t depth )
{
    ...
    // sampling texture
    ivec3_t node = ivec3(1, 1, width - 2);
    ...
    self->nodes = vector_new( sizeof(ivec3_t) );
    self->used = 0;
    self->width = width;
    self->height = height;
    self->depth = depth;
                   ...
                   const size_t height )
{
    ...
    node = (ivec3_t *) (vector_get( self->nodes, index ));
    x = node->x;
    y = node->y;
    width_left = width;
    i = index;
    ...
    while( width_left > 0 )
    {
	node = (ivec3_t *) (vector_get( self->nodes, i ));
	if( node->y > y )
	{
	    y = node->y;
	}
...
texture_atlas_merge( texture_atlas_t * self )
{
    ...
    {
        node = (ivec3_t *) (vector_get( self->nodes, i ));
        next = (ivec3_t *) (vector_get( self->nodes, i+1 ));
        if( node->y == next->y )
	{
            node->z += next->z;
                          ...
                          const size_t height )
{
    ...
    ivec3_t *node, *prev;
    ivec4_t region = ivec4(0, 0, width, height);
    size_t i;
    ...
    node = (ivec3_t *) malloc( sizeof(ivec3_t) );
    if( node == NULL) {
        freetype_gl_error( Out_Of_Memory );
        return ivec4(-1, -1, 0, 0);
        /* exit( EXIT_FAILURE ); */ /* Never exit from a library */
    ...
    {
        node = (ivec3_t *) vector_get( self->nodes, i );
        prev = (ivec3_t *) vector_get( self->nodes, i-1 );
