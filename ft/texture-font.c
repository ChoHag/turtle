...
static wchar_t convert_wint_to_wchar (wint_t value)
{
  /* Doesn't catch surrogates and other mistakes. */
  return ((value > 0 && value <= 0x10ffff) ? value : 0xfffd);
}
...
texture_glyph_get_kerning( const texture_glyph_t * self,
                           wint_t codepoint )
{
    wchar_t ucodepoint = convert_wint_to_wchar( codepoint );
    size_t i = ucodepoint >> 8;
    size_t j = ucodepoint & 0xFF;
    float *kern_index;
...
{
    if(codepoint == -1)
        return (texture_glyph_t *)self->atlas->special; /* Should find 0xfffd? */
    
    return texture_font_find_glyph_gi(self, codepoint);
}
...
cleanup_stroker:
    ...
    else if( ft_bitmap.pixel_mode == FT_PIXEL_MODE_BGRA && self->atlas->depth == 1 )
    {
        ...
        for( size_t row = 0; row < src_h; row++, dst_ptr += tgt_w * self->atlas->depth ) {
            for( size_t col = 0; col < src_w; col++, src++ ) {
        ...
        for( size_t row = 0; row < src_h; row++, dst += tgt_w ) {
            for( size_t col = 0; col < src_w; col++, src_ptr++ ) {
                dst[col] = (struct dst_pixel_t){ *src_ptr, *src_ptr, *src_ptr, 255 };
            }
        }
    }
    else
